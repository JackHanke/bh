...
class HDF5Dataset(Dataset):
    ...
    """
    def __init__(self, h5_path, percentages: (float, float), dataset_type: str, features_key='data', labels_key='labels'):
        self.h5_path = h5_path
        self.features_key = features_key
        self.labels_key = labels_key
        self.dataset_type = dataset_type
        self.trainpercentage = percentages[0]
        self.validpercentage = percentages[1]
        self.n = (config['end_dump'] - config['start_dump'])

        if self.dataset_type == 'train':
            self.offset = 0
            self.size = int(self.n*(self.trainpercentage))
        elif self.dataset_type == 'valid':
            self.offset = int(self.n*(self.trainpercentage))
            self.size = int(self.n*(self.validpercentage))
        elif self.dataset_type == 'test':
            self.offset = int(self.n*(self.trainpercentage + self.validpercentage))
            self.size = int(self.n - (self.n*(self.trainpercentage + self.validpercentage)))
    ...
    def __len__(self):
        ...
        # with h5py.File(self.h5_path, 'r') as f:
            ...
            # assert self.size == actual_size, "Number of dumps in config does not match HDF5 File."
        return self.size
